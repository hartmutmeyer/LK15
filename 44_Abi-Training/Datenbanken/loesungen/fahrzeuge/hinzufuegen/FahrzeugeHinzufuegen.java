			...
			if (rs.next()) {
				JOptionPane
						.showMessageDialog(this,
								"Es gibt bereits ein angemeldetes Fahrzeug mit diesem KFZ-Zeichen!");
			} else {
		...
		if (kfzZeichenNochNichtVergeben) {
			...
			if (ergebnis == 1) {
				JOptionPane.showMessageDialog(this,
						"Das Fahrzeug mit dem KFZ-Zeichen "
								+ textFieldKFZKennzeichen.getText()
								+ " wurde hinzugefügt.");
			} else {
				System.out
						.println("Es gibt bereits ein angemeldetes Fahrzeug mit diesem KFZ-Zeichen!");
			}
		}
	}
	...
	private void datenbankOeffnen() {
		try {
			conn = DriverManager.getConnection(
					"jdbc:mysql://localhost/fahrzeuge", "root", "root");
		} catch (SQLException e) {
			System.out.println(e.getMessage());
			e.printStackTrace();
		}
